...
contract SHAMBALUV is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MIN_TIMELOCK_DELAY = 1 hours; // Minimum 1 hour timelock
     ...
     */
    function setMaxTransferAmount(uint256 _newMax) external onlyOwner timelock(timelockDelay) {
     ...
     */
    function setTeamWallet(address _teamWallet) external onlyOwner timelock(timelockDelay) {
    ...
    function setLiquidityWallet(address _liqWallet) external onlyOwner timelock(timelockDelay) {
     ...
     * @return minDelay Minimum allowed delay
