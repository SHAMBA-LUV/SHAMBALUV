...
contract SHAMBALUV is ERC20, Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_TIMELOCK_DELAY = 1000 * 365 days; // Maximum 1000 years timelock
    uint256 public constant DEFAULT_TIMELOCK_DELAY = 24 hours; // Default 24 hour timelock
    ...
    // ============ PROPER TIMELOCK FUNCTIONS ============
    
    /**
     * @dev Propose a timelocked operation
     * @param operationId Unique identifier for the operation
     * @param delay Delay in seconds before operation can be executed
     * @notice Only owner can propose operations
     */
    function proposeTimelockOperation(bytes32 operationId, uint256 delay) external onlyOwner {
        require(operationId != bytes32(0), "Invalid operation ID");
        require(delay <= MAX_TIMELOCK_DELAY, "Delay too long");
        require(operationStates[operationId] == OperationState.Unset, "Operation already exists");
     ...
     * @param operationId Unique identifier for the operation
     * @notice Only owner can execute operations
     */
    function executeTimelockOperation(bytes32 operationId) external onlyOwner {
        require(operationStates[operationId] == OperationState.Waiting, "Operation not waiting");
        
        uint256 executionTime = timelockProposals[operationId];
        require(block.timestamp >= executionTime, "Timelock: Delay not met");
        
        operationStates[operationId] = OperationState.Ready;
        emit TimelockExecuted(operationId, msg.sender);
    }
    
    /**
     * @dev Cancel a timelocked operation
     * @param operationId Unique identifier for the operation
     * @notice Only owner can cancel operations
     */
     ...
     */
    function isTimelockOperationReady(bytes32 operationId) external view returns (bool ready) {
        if (operationStates[operationId] != OperationState.Waiting) {
            return false;
        }
     ...
     * @param operationId Unique identifier for the operation
     * @return state Current operation state
     * @return executionTime When the operation can be executed (if waiting)
     */
    function getTimelockOperation(bytes32 operationId) external view returns (
    ...
    /**
     * @dev Mark operation as done (called after successful execution)
     * @param operationId Unique identifier for the operation
     * @notice Only owner can mark operations as done
     */
    function markTimelockOperationDone(bytes32 operationId) external onlyOwner {
        require(operationStates[operationId] == OperationState.Ready, "Operation not ready");
